...
test('.only() - Single String Parameter', function(assert) {
    ...
    Query
        ...
        .then(function success(entries) {
            // assert.ok("entries" in result, 'Entries key present in the resultset');
            var flag = entries[0].every(function(entry) {
                return (entry && Object.keys(entry).length === 4 && "title" in entry && "uid" in entry && "url" in entry);
            });
...
test('.only() - Array Parameter', function(assert) {
    ...
    Query
        ...
        .then(function success(entries) {
            // assert.ok("entries" in result, 'Entries key present in the resultset');
            var flag = entries[0].every(function(entry) {
                return (entry && Object.keys(entry).length === 4 && "title" in entry && "url" in entry && "uid" in entry);
            });
    ...
    Query
        ...
        .then(function success(entries) {
            ...
            if (entries && entries.length && entries[0].length) {
                ...
                assert.equal(_entries, true, "Modular Block entries sorted descending on '" + field + "' field");
            }
            assert.end();
        }, function error(err) {
            console.error('Error : ', err);
    ...
    Query
        ...
        .then(function success(entries) {
            ...
            if (entries && entries.length && entries[0].length) {
                ...
                assert.equal(_entries, true, "Modular Block entries sorted ascending on '" + field + "' field");
            }
            assert.end();
        }, function error(err) {
            assert.fail(".greaterThan()");
    ...
    Query
        ...
        .then(function success(entries) {
            ...
            if (entries && entries.length && entries[0].length) {
                ...
                assert.equal(_entries, false, "Modular Block entries sorted descending on '" + "' field");
            }
            assert.end();
        }, function error(err) {
            console.error("error :", err);
...
test('.or() - Query Objects', function(assert) {
    var Query1 = Stack.ContentType(contentTypes.source).Query().where('modular_blocks.test1.single_line', 'Rohit');
    var Query2 = Stack.ContentType(contentTypes.source).Query().where('modular_blocks.test1.number', 10);
    var Query =  Stack.ContentType(contentTypes.source).Query();
...
test('.regex()', function(assert) {
    ...
    Query
        ...
        .then(function success(entries) {
            ...
            assert.ok(flag, "Modular Block regexp satisfied for all the entries in the resultset");
            assert.end();
        }, function error(err) {
            console.error("error :", err);
            assert.fail(".regex()");
